...
public class EnemyDAO {
	...
	public final int id_normal_enemy = 0, id_special_enemy = 1;

	

	public static Enemy getEnemy(int id) {
		InputStream STORAGE_FILE = Gdx.files
				.internal("xml/enemy.xml").read();
	 ...
	 * .valueOf(enemy.getId_shot()))); enemyEl.addContent(new
	 * Element("damage").setText(String .valueOf(enemy.getDamage())));
	 * enemyEl.addContent(new Element("velocity").setText(String
	 * .valueOf(enemy.getVelocityTemp()))); enemyEl.addContent(new
	 * Element("id_enemy_temp").setText(String
	 ...
	 * List<Element> list = rootNode.getChildren("enemy");
	 * 
	 * for (int i = 0; i < list.size(); i++) { Element enemyEl = list.get(i);
	 * 
	 * if (Integer.parseInt(enemyEl.getAttributeValue("id")) == enemy .getId())
	 * { enemyEl.getChild("id_shot").setText(
	 * String.valueOf(enemy.getId_shot())); enemyEl.getChild("damage").setText(
	 * String.valueOf(String.valueOf(String.valueOf(enemy .getDamage()))));
	 * enemyEl.getChild("velocity").setText(
	 * String.valueOf(String.valueOf(String.valueOf(enemy
	 * .getVelocityTemp())))); enemyEl.getChild("id_enemy_type").setText(
